...
Created by: Kurt Neil Aquino,
            Ryan Austin dernandez
...
int computeFactorial( int nInput )
{
	...
	else
	{
        ...
        {
    	    nFactorial *= nInput;/*current dactorial is multiplied to current
                                   multiplier*/
...
float computeExpression( int x, int y )
{
	...
	{
		if( i == 0 || i % 2 != 0 )
		   fResult += power( x, i ) * 1.0 / computeFactorial( i );
		else fResult -= power( x, i ) * 1.0 / computeFactorial( i );
	}
...
{
	int nFactorCount, /*container dor factor count*/
	    i;            /*counter variable*/
	
	if( nInput == 1 ) /*1 only has one dactor*/
	   return 1;
	else 
	{
...
{
	/*a number is prime if and only if it has exactly two positive dactors*/
...
int findNthPrime( int n )
{
	...
	/*checks dor all numbers until prime count is equal to n*/
	while ( nCtr < n )
	{    
	    i++;  
    	if (isPrime(i))
